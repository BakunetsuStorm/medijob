...
const JobCard = ({ job }) => {
  ...
  const getCategoryGradient = (category) => {
    const gradients = {
      ...
      'Ачигч': 'from-slate-600 to-slate-800', 
    };
    return gradients[category] || 'from-indigo-600 to-blue-800'; 
  };

  // 2. Ажил олгогчийн нэрний эхний үсгийг авах
  ...
  return (
    ...
    >
      ...
      <div className={`relative h-48 bg-gradient-to-br ${gradientClass} overflow-hidden`}>
        ...
        {/* Category Tag */}
        <div className="absolute top-4 left-4 z-20 bg-white/95 dark:bg-black/80 backdrop-blur-md px-3.5 py-1.5 rounded-full text-xs font-bold text-gray-900 dark:text-gray-100 shadow-sm uppercase tracking-wider">
          {job.category}
        </div>
      </div>
      ...
      <div className="p-6 flex flex-col flex-grow relative">
        ...
        <div className="flex items-center gap-3 mb-6 mt-auto">
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-black text-sm bg-gray-900 dark:bg-gray-700 border-2 border-white dark:border-[#1a1a1a] shadow-sm ring-1 ring-gray-100 dark:ring-gray-800">
            {employerInitial}
          </div>
        ...
        <div className="pt-5 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
            ...
            <span className="text-sm font-bold text-yellow-700 dark:text-yellow-500">{job.rating || "Шинэ"}</span>
          </div>
          ...
          <div className="text-right flex flex-col">
            ...
            <span className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide">/{job.salaryType}</span>
          </div>
