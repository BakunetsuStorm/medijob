...
const LOCATION_DATA = {
  "Улаанбаатар": {
    ...
    "Сүхбаатар дүүрэг (СБД)": 20, "Хан-Уул дүүрэг (ХУД)": 25, "Чингэлтэй дүүрэг (ЧД)": 24
  }
};
...
const AddJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isSalaryTooLow) {
      Swal.fire({ title: 'Анхааруулга', text: `Цалингийн хэмжээ доод тал нь ${minRequired.toLocaleString()}₮ байх ёстой.`, icon: 'warning', confirmButtonColor: '#3b82f6' });
      return;
    }

    if (formData.isRecurring && formData.recurringDays.length === 0) {
      Swal.fire({ title: 'Мэдээлэл дутуу', text: 'Тогтмол давтамжтай ажил тул ядаж 1 гараг сонгоно уу!', icon: 'warning', confirmButtonColor: '#f59e0b' });
      return;
    }
    ...
    if (formData.locationType !== 'Зайнаас') {
      if (!formData.district || !formData.khoroo || !formData.specificAddress) {
        Swal.fire({ title: 'Хаяг дутуу', text: 'Байршлын мэдээллийг гүйцэд оруулна уу.', icon: 'warning', confirmButtonColor: '#f59e0b' });
        return;
      }
    ...
    if (formData.isFlexibleTime) {
      ...
      finalWorkingHours = `${formData.startTime} - ${formData.endTime}`;
    }
    ...
    try {
      ...
      Swal.fire({ title: 'Амжилттай!', text: 'Ажлын зар амжилттай нийтлэгдлээ!', icon: 'success', confirmButtonText: 'Нүүр хуудас руу буцах', confirmButtonColor: '#10b981' }).then(() => { navigate('/'); });
    } catch (error) {
      Swal.fire({ title: 'Алдаа гарлаа', text: 'Зар нийтлэхэд алдаа гарлаа, та мэдээллээ шалгана уу.', icon: 'error', confirmButtonColor: '#ef4444' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white dark:bg-[#111111] rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden transition-colors duration-300">
          ...
          <form onSubmit={handleSubmit} className="p-8 sm:p-12 space-y-10">
            ...
            <div>
              ...
              <div className="p-6 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-2xl transition-colors">
                <div className="flex items-center gap-3 mb-5">
                  <input 
                    type="checkbox" 
                    id="isFlex"
                    checked={formData.isFlexibleTime}
                    onChange={(e) => setFormData({...formData, isFlexibleTime: e.target.checked, startTime: '', endTime: ''})}
                    className="w-5 h-5 text-blue-600 rounded cursor-pointer"
                  />
                  <label htmlFor="isFlex" className="font-bold text-blue-900 dark:text-blue-400 cursor-pointer">
                     Уян хатан цагийн хуваарьтай (Тохиролцох боломжтой)
                  </label>
                </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input 
                          ...
                          onChange={handleChange}
                          className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-[#1a1a1a] dark:text-white border border-blue-200 dark:border-blue-800/50 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all font-black text-lg cursor-pointer"
                        />
                      </div>
                    </div>
                    <div>
            ...
            {/* 5. Түр зуурын болон Давтамжтай ажил */}
            <div className="space-y-4">
              <div className="p-5 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-900/50 rounded-xl transition-colors">
                <div className="flex items-center gap-3 mb-3">
                  <input type="checkbox" id="isTemp" checked={formData.isTemporary} onChange={(e) => setFormData({...formData, isTemporary: e.target.checked})} className="w-5 h-5 text-orange-600 rounded cursor-pointer" />
                  ...
                  <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="date" value={formData.tempStartDate} onChange={(e) => setFormData({...formData, tempStartDate: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-[#1a1a1a] border border-orange-200 dark:border-orange-700/50 rounded-xl outline-none focus:ring-2 focus:ring-orange-500 dark:text-white transition-all cursor-pointer" required={formData.isTemporary} />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Дуусах өдөр</label>
                        <input type="date" value={formData.tempEndDate} min={formData.tempStartDate} onChange={(e) => setFormData({...formData, tempEndDate: e.target.value})} className="w-full px-4 py-3 bg-white dark:bg-[#1a1a1a] border border-orange-200 dark:border-orange-700/50 rounded-xl outline-none focus:ring-2 focus:ring-orange-500 dark:text-white transition-all cursor-pointer" required={formData.isTemporary} />
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="p-5 bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-900/50 rounded-xl transition-colors">
                <div className="flex items-center gap-3 mb-3">
                  <input type="checkbox" id="isRecurr" checked={formData.isRecurring} onChange={(e) => setFormData({...formData, isRecurring: e.target.checked})} className="w-5 h-5 text-teal-600 rounded cursor-pointer" />
                  ...
                  <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {WEEKDAYS.map(day => {
                        ...
                        return (
                          <button key={day} type="button" onClick={() => toggleRecurringDay(day)} className={`px-4 py-2 rounded-lg text-sm font-bold transition-all border ${isSelected ? 'bg-teal-600 text-white border-teal-600 shadow-md' : 'bg-white dark:bg-[#1a1a1a] text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-teal-400'}`}>
                            {isSelected ? '✓ ' : ''}{day}
                          </button>
                        )
                      })}
